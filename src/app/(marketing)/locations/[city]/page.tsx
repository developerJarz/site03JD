...
import { getProjects, getServices, getSiteSettings } from "@/lib/data/public";
import { faqSchema } from "@/lib/seo";
import { locationOffices, officePath, officeSlug } from "@/lib/seo/locations";
...
/** Projects for clients in this office’s city or country. */
function localProjects(o: Office, projects: Project[]) {
  const needles = [o.city, o.country].map((n) => n.toLowerCase());
  return projects.filter((p) => needles.some((n) => p.location?.toLowerCase().includes(n)));
}
...
function locationFaqs(o: Office, services: Service[], settings: SiteSettings, local: Project[]) {
  ...
  if (local.length > 0) {
    faqs.push({
      question: `Have you worked with businesses in ${o.country === "United States" ? o.city : o.country}?`,
...
export default async function LocationPage({ params }: PageProps<"/locations/[city]">) {
  const { city } = await params;
  const [{ settings, office }, services, projects] = await Promise.all([findOffice(city), getServices(), getProjects()]);
  if (!office) notFound();

  const place = officePlace(office);
  const local = localProjects(office, projects);
  const work = local.length > 0 ? local.slice(0, 4) : projects.filter((p) => p.featured).slice(0, 4);
  const faqs = locationFaqs(office, services, settings, local);
  ...
  return (
    <>
        ...
        aside={
          office.image ? (
            <div className="relative aspect-[4/3] overflow-hidden rounded-[32px] border border-white/10 lg:ml-auto lg:max-w-md">
              <Image src={office.image.src} alt={office.image.alt} fill priority sizes="(min-width: 1024px) 28rem, 100vw" className="object-cover" />
            </div>
          ) : (
      ...
      {work.length > 0 && (
        <Section tone="light" aria-labelledby="work-heading">
          <SectionHeader
            index="03"
            eyebrow="Portfolio"
            title={<span id="work-heading">{local.length > 0 ? `Work for businesses in ${office.country === "United States" ? office.city : office.country}.` : "Recent work."}</span>}
            description={local.length > 0 ? undefined : `Projects our team has delivered for clients in other markets.`}
          />
      ...
      {/* FAQ */}
      <Section tone={work.length > 0 ? "mist" : "light"} aria-labelledby="faq-heading">
